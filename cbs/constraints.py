...
class Constraints:
    ...
    '''
    def fork(self, agent: Agent, obstacles: Dict[int, Set[Tuple[int, int]]]) -> 'Constraints':
        agent_constraints_copy = deepcopy(self.agent_constraints)
        for time, obstacle in obstacles.items():
            agent_constraints_copy.setdefault(agent, dict()).setdefault(time, set()).update(obstacle)
